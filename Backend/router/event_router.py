...
async def update_event_endpoint(event_id: str, event: EventUpdate, current_user: dict = Depends(get_current_user)):
    user_id = current_user["id"]
...
async def delete_event_endpoint(event_id: str, current_user: dict = Depends(get_current_user)):
    user_id = current_user["id"]
